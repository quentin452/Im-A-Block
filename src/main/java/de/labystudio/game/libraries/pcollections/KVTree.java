 ...
 *
 * <p>
 * All operations are guaranteed to complete within O(log n) time. A complete iteration pass over
...
final class KVTree<K, V> implements Map.Entry<K, V>, Serializable {
     ...
     *
     * <p>
     * Requires time proportional to the difference in heights between 'left' and 'right', which is
     * in O(log(max(left.size, right.size))) = O(log(left.size + right.size)).
     *
     * <p>
     * The height of the returned tree is guaranteed to be max(left.height, right.height) plus
     ...
     */
    private static <K, V> KVTree<K, V> join(final KVTree<K, V> left, final K key, final V value,
    ...
    private static <K, V> KVTree<K, V> fromIterator(final Iterator<?> iterator, final IteratorType iteratorType,
        final int maxHeight) {
        ...
        while (true) {
            ...
            @SuppressWarnings("unchecked")
            final K key = (K) (iteratorType == IteratorType.KEY ? datum : ((Map.Entry<?, ?>) datum).getKey());
            @SuppressWarnings("unchecked")
            final V value = (V) (iteratorType == IteratorType.KEY ? null : ((Map.Entry<?, ?>) datum).getValue());
    ...
    KVTree<K, V> range(final K leftBound, final boolean isLeftBoundInclusive, final K rightBound,
        final boolean isRightBoundInclusive, final Comparator<? super K> comparator) {
        if (this.isEmpty()) {
            return this;
        }
        ...
        if (toCmp < 0) {
            ...
            if (fromCmp == 0) {
                ...
                final KVTree<K, V> left = this.left.rangeToRight(leftBound, isLeftBoundInclusive, comparator);
            ...
            final KVTree<K, V> right = this.right.rangeToLeft(rightBound, isRightBoundInclusive, comparator);
    ...
    KVTree<K, V> rangeToLeft(final K rightBound, final boolean isRightBoundInclusive,
    ...
    KVTree<K, V> rangeToRight(final K leftBound, final boolean isleftBoundInclusive,
        final Comparator<? super K> comparator) {
        if (this.isEmpty()) {
            return this;
        }
    ...
    KVTree<K, V> search(final K key, final Comparator<? super K> comparator, final SearchType searchType) {
        KVTree<K, V> currNode = this;
        ...
        while (true) {
            ...
            if (cmp < 0) {
                ...
                } else if (searchType == SearchType.LE || searchType == SearchType.EQ || searchType == SearchType.GE) {
                    return currNode;
                } else {
