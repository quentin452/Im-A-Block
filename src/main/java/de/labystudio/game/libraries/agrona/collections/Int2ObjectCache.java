...
import static de.labystudio.game.libraries.agrona.collections.CollectionUtil.validatePositivePowerOfTwo;
...
public class Int2ObjectCache<V> implements Map<Integer, V> {
     ...
     */
    public Int2ObjectCache(@DoNotSub final int numSets, @DoNotSub final int setSize,
    ...
    public boolean containsKey(final int key) {
        ...
        for (@DoNotSub
        int i = setBeginIndex, setEndIndex = setBeginIndex + setSize; i < setEndIndex; i++) {
            if (null == values[i]) {
                break;
            }
     ...
     * {@link IntFunction} and put it in the cache.
     * <p>
    ...
    public V put(final int key, final V value) {
        ...
        for (@DoNotSub
        int nextSetIndex = setBeginIndex + setSize; i < nextSetIndex; i++) {
    ...
    public V remove(final int key) {
        ...
        Object value = null;
        for (@DoNotSub
        int i = setBeginIndex, nextSetIndex = setBeginIndex + setSize; i < nextSetIndex; i++) {
            value = values[i];
            if (null == value) {
                break;
            }
    ...
    private void shuffleDown(final int setBeginIndex) {
        ...
        final Object[] values = this.values;
        for (@DoNotSub
        int i = setBeginIndex + (setSize - 1); i > setBeginIndex; i--) {
            values[i] = values[i - 1];
            keys[i] = keys[i - 1];
        }
    ...
    public void putAll(final Int2ObjectCache<? extends V> map) {
        final Int2ObjectCache<? extends V>.EntryIterator iterator = map.entrySet()
            .iterator();
    ...
    abstract class AbstractIterator<T> implements Iterator<T> {
        ...
        protected final void findNext() {
            ...
            for (@DoNotSub
            int i = position + 1, size = capacity; i < size; i++) {
                if (null != values[i]) {
                    found = true;
                    position = i;
                    --remaining;
