...
import static de.labystudio.game.libraries.agrona.collections.Hashing.compoundKey;
...
public class BiInt2ObjectMap<V> {
     ...
     */
    public V computeIfPresent(final int keyPartA, final int keyPartB,
     ...
     */
    public V compute(final int keyPartA, final int keyPartB,
        final EntryRemap<? super V, ? extends V> remappingFunction) {
        final long key = compoundKey(keyPartA, keyPartB);
        requireNonNull(remappingFunction);
        final long[] keys = this.keys;
     ...
     */
    public V merge(final int keyPartA, final int keyPartB, final V value,
